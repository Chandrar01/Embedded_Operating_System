...
class PageManagement:
    ...
    def AddOrReplacePageInMainMemory(self, main_memory, pte, process_memory_pair):
        ...
        if self.index >= MAIN_MEMORY_SIZE:
            # remove one page from main memory and add one page
            # identify FIFO page using popleft() method of dequeue
            # and remove that page from main_memory
            page_to_remove = main_memory.popleft()
            # remove the page entry from set of process_memory_pair list too
            process_memory_pair.remove((page_to_remove.process_id, page_to_remove.page_vpn))

            # add new pte to the main memory
